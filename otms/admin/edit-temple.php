...
if (strlen($_SESSION['aid']) == 0) {
    ...
    if(isset($_POST['submit'])) {
        ...
        $stmt = $con->prepare("UPDATE tbltemple SET TempleName=?, TempleLocation=?, City=?, State=?, Country=?, Description=? WHERE ID=?");
        
        // Bind parameters
        $stmt->bind_param("ssssssi", $tname, $tlocation, $city, $state, $country, $description, $id);
        ...
        if($stmt->affected_rows > 0) {
            ...
            echo "<script>alert('Error updating temple detail');</script>";
        }
...
<head>
  ...
  <title>Online Temple Seva || Update Temple</title>
  <!-- base:css -->
